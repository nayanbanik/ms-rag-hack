...
async def get_summary(client, prompt, data):
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                ...
                { "role": "user", "content": [  
                    { 
                        "type": "text", 
                        "text": f"Analyze the image and provide a detailed description of it. Ensure the description supports answering the user's question: '{prompt}'."
                    },
                    { 
                        "type": "image_url",
                        "image_url": {
...
@authenticated
async def upload_image(auth_claims: Dict[str, Any]):
    ...
    prompt = form_data.get("question", "")

    if not prompt:
        return jsonify({"error": "No question provided"}), 400

    # You now have both the file and the question, process them as needed
    ...
    api_key = os.getenv("AZURE_IMAGE_INFERENCING_OPENAI_KEY")
    api_version = os.getenv("AZURE_IMAGE_INFERENCING_API_VERSION")
    endpoint = os.getenv("AZURE_IMAGE_INFERENCING_ENDPOINT")
    img_txt = ""

    async with AsyncAzureOpenAI(api_version=api_version, azure_endpoint=endpoint, api_key=api_key) as client:
        with open(file_path, "rb") as fp:
            image_data = fp.read()
            image_base64 = base64.b64encode(image_data).decode('utf-8')
            image_data_url = f"data:image/png;base64,{image_base64}"
            img_txt = await get_summary(client, prompt, image_data_url)
            print(img_txt)

        if os.path.exists(file_path):
            os.remove(file_path)
    return jsonify({"message": str(img_txt)}), 200
